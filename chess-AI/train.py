...
class TrainOptions:
    ...
    def __init__(self, games, lr, epochs, batch_size):
        self.lr = lr
        self.games = games
        self.epochs = epochs
...
class Train:
    """This class is used to run the model training.

    Attributes:
        move_approximator: Gets approximate best moves from a position
        value_approximator: Gets approximate value of a position
        policy_converter: References the PlayNetworkPolicyConverter class
    """
    def __init__(self, move_approximator, save_path, val_approximator=None):
    ...
    def training_game(self):
        ...
        while True:
            ...
            # Converts move probabilites to (8,8,73) vector
    ...
    def training_episode(self, nnet, options, num_saved_models, overwrite_save):
        """Builds dataset from given number of training games and current network,
        then trains the network on the output and game outcomes.
        """

        with torch.no_grad():
            # Obtain data from games and separate into appropriate lists
            game_data = [self.training_game() for _ in range(options.games)]
            ...
            opt = torch.optim.SGD(nnet.parameters(), lr=options.lr, weight_decay=0.001, momentum=0.9)
...
def main():
    ...
    options = TrainOptions(games=3, lr=0.2, epochs=3, batch_size=10)
    train = Train(move_approximator=mcts_moves, save_path=None)

    train.training_episode(nnet, options, num_saved_models=num_saved_models, overwrite_save=overwrite_save)
