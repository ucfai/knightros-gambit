...
class BoardStateDetector:
    ...
    def get_board_corners(board_img):
        pass
        '''
        I have a few corner detection algorithms here, I think overall canny is our best bet, although everything I'm reading also mentions Harris so we can possibly tinker again with it.
        Shi-Tomasi Corner is designed to calculated the R value better than Harris so it lends to better results & sobel is moreso for gradients but can be used to detect corners.
        '''
        # Sobel Edge Detector, codes is from the docs: https://docs.opencv.org/3.4/d5/d0f/tutorial_py_gradients.html 
        '''
        board_img = 'board_img.jpg'
        img = cv2.imread(board_img)
        laplacian = cv.Laplacian(img,cv.CV_64F)
        sobelx = cv.Sobel(img, cv.CV_64F, 1, 0, ksize = 5) # Vertical mask
        sobely = cv.Sobel(img, cv.CV_64F, 0, 1, ksize = 5) # Horizontal mask
        ...
        plt.show()
        '''
        # Shi-Tomasi Corner Detector, code is from the docs: https://docs.opencv.org/3.0-beta/doc/py_tutorials/py_feature2d/py_shi_tomasi/py_shi_tomasi.html
        # More about it: https://medium.com/pixel-wise/detect-those-corners-aba0f034078b
        '''
        board_img = 'board_img.jpg'
        img = cv2.imread(board_img)
        ...
        '''
        # Canny Edge Detection, code is from the docs: https://docs.opencv.org/3.4/da/d22/tutorial_py_canny.html
        '''
        board_img = 'board_img.jpg'
        img = cv.imread(board_img,0)
        edges = cv.Canny(img,100,200)  
        # I've not seen anything entirely concrete as to how to find the minVal and maxVal so this should be discussed if others have thoughts
        # I did find this though https://stackoverflow.com/questions/35102372/opencv-canny-do-minval-and-maxval-matter-if-youre-working-with-a-black-and-whi
