...
class Game:
    ...
    def last_made_move(self):
        ...
        '''if self.board.engine.chess_board.move_stack:
            return None'''#this was always defaulting to true and not allowing access to move
    ...
    def process(self, player):
        ...
        """
        print("this is our player:", player)
        print("the turn is white: ", self.board.engine.is_white_turn())
        print("this is the message queue: ", self.board.msg_queue)
        arduino_status = self.board.get_status_from_arduino()
        ...
        if arduino_status.status == status.ArduinoStatus.IDLE:
            ...
            if self.board.msg_queue:
                print("we are using a move from the queue")
    ...
    def processM(self, move):
            ...
            if arduino_status.status == status.ArduinoStatus.IDLE:
                ...
                # Takes the move as input for function
                msg = move
